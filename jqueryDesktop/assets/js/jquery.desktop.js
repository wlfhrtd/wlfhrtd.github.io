...
jQuery(document).ready(function() {
  let $about = $("#aboutTextArea");
  let about = $about.val();
  let replace = "#GET_AGE";
  let BoD = "1990/07/17";
  let n = about.replace(replace, getAge(BoD));
  $about.val(n);
// start btn
  let $startBtn = $("#start_button");
  let $startMenu = $("#start_menu_wrapper");

  $startBtn.click(function () {
    if (!$startBtn.hasClass('pressed')) {
      $startBtn.attr('src', "assets/images/icons/icon_22_desktop_pressed.png");
      $startBtn.addClass('pressed');

      $startMenu.css("visibility", "visible");
    } else {
      $startBtn.attr('src', "assets/images/icons/icon_22_desktop.png");
  ...
  $("#powerBtn").click(function () {
    $("#wallpaper").attr('src', "assets/images/misc/bsod.png").attr('class', 'stretched').attr("z-index", 10);
    $("#wrapper").remove();
  });

  const startup = new Audio("assets/sounds/Windows XP Startup.wav");
  startup.play();
